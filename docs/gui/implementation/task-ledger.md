...
| OM-280 | `codex/om280-timeline-projection` | `F:\\claude\\projects\\OpenManic\\.agents\\om280-timeline-projection` | `be641cc` | `crates/openmanic-application/src/{lib.rs,projection.rs,timeline_projection.rs}` | Serialized application-projection allocation after OM-220; no storage, UI, platform, root-manifest, or domain path is transferred | Active; Phase 2 verification deferred to the one phase gate |
| OM-240 | `codex/om240-windows-control-loop` | `F:\\claude\\projects\\OpenManic\\.agents\\om240-windows-control-loop` | `be641cc` | `crates/openmanic-platform/{Cargo.toml,src/{lib.rs,windows_control.rs,windows_raw.rs}}` | Windows API dependency/version and lockfile remain primary-owned; task begins after that primary preparation | Queued; Phase 2 verification deferred to the one phase gate |
