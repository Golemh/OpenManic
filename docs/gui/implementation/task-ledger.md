...
| OM-400 | `codex/om400-overview-projection` | `F:\\claude\\projects\\OpenManic\\.agents\\om400-overview-projection` | pending primary batch base | `crates/openmanic-application/src/overview.rs` | The primary owns `lib.rs` and all public re-exports. OM-400 is a pure cancellable projection model and must not alter storage, migrations, UI, composition, manifests, or the saved-view contract reserved for OM-401. | Active: Phase 4 Overview projection foundation |
| OM-410 | `codex/om410-calendar-projection` | `F:\\claude\\projects\\OpenManic\\.agents\\om410-calendar-projection` | pending primary batch base | `crates/openmanic-application/src/calendar_projection.rs` | The primary owns `lib.rs`, correlated storage read records, and composition wiring. OM-410 must reuse schedule occurrence contracts and must not alter storage, migrations, UI, composition, or manifests. | Active: Phase 4 Calendar projection foundation |
