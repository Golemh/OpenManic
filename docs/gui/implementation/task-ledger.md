...
| OM-330 | primary integration branch | `D:\\y-Coding\\Human_Coding\\Codex_Hackathon\\OpenManic` | `d8e91f8` | `crates/openmanic-domain/src/{ids.rs,lib.rs,schedule.rs}`, `crates/openmanic-application/src/{schedule.rs,schedule_time.rs,lib.rs}`, `crates/openmanic-storage-sqlite/src/{migration.rs,writer.rs}`, `crates/openmanic-storage-sqlite/migrations/0002_schedule_exception_boundary_resolution.sql` | Primary-owned schedule authority: the existing persistence foundation now creates, replaces, and deletes atomically with revision checks; it rejects one-time/one-time, one-time/recurring, and recurring/recurring positive intersections using the IANA-aware expansion boundary, persisted exceptions, and half-open adjacency semantics. | In progress. Focused storage/application tests and Clippy pass. Remaining: structured conflicting-occurrence results, service-level edit-scope commands/reconciliation, then OM-331's read projection and overlays. |
