...
| OM-330 | primary integration branch | `D:\\y-Coding\\Human_Coding\\Codex_Hackathon\\OpenManic` | `809fd15` | `crates/openmanic-domain/src/{ids.rs,lib.rs,schedule.rs}`, `crates/openmanic-application/src/{schedule.rs,lib.rs}`, `crates/openmanic-storage-sqlite/src/{migration.rs,writer.rs}`, `crates/openmanic-storage-sqlite/migrations/0002_schedule_exception_boundary_resolution.sql` | Primary-owned schedule foundation: distinct IDs, validated persistence-safe civil segments, occurrence-only exceptions with DST provenance, immutable commands/service, migration-backed atomic SQLite creation, category references, and duplicate-ID rejection. | Focused-verified. Remaining OM-330 work is authoritative cross-schedule overlap validation; it depends on the approved IANA resolver/expansion boundary. OM-331 begins only after that authority is completed. |
