...
| OM-330 | primary integration branch | `D:\\y-Coding\\Human_Coding\\Codex_Hackathon\\OpenManic` | `af4d1a1` | `crates/openmanic-domain/src/{ids.rs,lib.rs,schedule.rs}` | Primary-owned schedule contract foundation: distinct stable IDs for recurring series and one-time items plus validated persistence-safe civil segments and restore of non-overlapping series lineage. The immutable schema's public IDs and civil fields remain the source of truth. | Integrated and focused-verified; schedule persistence/service, authoritative overlap validation, exceptions, and edit scopes continue in this ordered batch |
