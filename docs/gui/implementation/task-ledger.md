...
| OM-220 | `codex/om220-storage-repositories` | `F:\\claude\\projects\\OpenManic\\.agents\\om220-storage-repositories` | `58a73f3` | `crates/openmanic-storage-sqlite/src/{connection.rs,errors.rs,lib.rs,repository.rs,writer.rs}` | No schema/migration, application, root manifest, UI, or platform path is transferred | Active after OM-151 and OM-210; Phase 2 verification deferred to the one phase gate |
| OM-230 | `codex/om230-platform-normalization` | `F:\\claude\\projects\\OpenManic\\.agents\\om230-platform-normalization` | `58a73f3` | `crates/openmanic-platform/src/{lib.rs,adapter.rs,capabilities.rs,fake.rs}` | Consumes the accepted typed tracking evidence; no application, storage, UI, root manifest, or Windows FFI path is transferred | Active in parallel with OM-220; Phase 2 verification deferred to the one phase gate |
