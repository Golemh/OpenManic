...
pub fn application_brand_color(name: &str) -> Option<Color32> {
    ...
    } else if lower.contains("vs code") || lower.contains("visual studio code") || lower.contains("code")
    {
...
pub fn segment_option(ui: &mut egui::Ui, text: &str, selected: bool) -> bool {
    ...
    ui.add(
        egui::Button::new(egui::RichText::new(text).size(12.5).strong().color(text_color))
            .fill(fill)
            .stroke(Stroke::NONE)
            .corner_radius(CornerRadius::same(RADIUS_BUTTON)),
    )
